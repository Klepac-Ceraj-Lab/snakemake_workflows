...
rule humann:
    ...
    run:
        shell("humann --input {{input.seq}} --taxonomic-profile {{input.tax}} --output {} --threads 16 --remove-temp-output --search-mode uniref90 --output-basename {{wildcards.sample}} --metaphlan-options '-x mpa_v296_CHOCOPhlAn_201901'".format(
