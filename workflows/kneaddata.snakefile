...
rule kneaddata_filter:
    ...
    output:
        fwd = expand(os.path.join(os.sep, kneadfolder, "{samples}_R1_001_kneaddata_paired_1.fastq"), samples = SAMPLES),
        rev = expand(os.path.join(os.sep, kneadfolder, "{samples}_R1_001_kneaddata_paired_2.fastq"), samples = SAMPLES)
    run:
